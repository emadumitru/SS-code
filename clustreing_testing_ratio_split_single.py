...
def get_action_list(lines):
    ...
    if current_block:  # Ensure the last block is added
        blocks.append(current_block)
...
# Function to extract action ratios from sequences
def extract_action_ratios(sequences):
    features = []
    for seq in sequences:
        insert_node = seq.count('insert-node')
        move_tree = seq.count('move-tree')
        delete_node = seq.count('delete-node')
        update_node = seq.count('update-node')
        total_actions = len(seq)
        
        if total_actions == 0:
            features.append([0, 0, 0, 0])
        else:
            features.append([insert_node / total_actions, move_tree / total_actions, delete_node / total_actions, update_node / total_actions])
    
    return np.array(features)

# Function to prepare data with minimal edit action sequences and extract action ratios
...
def run_experiments_sequences(merged_df, refactorings, models, param_grids, test_splits, random_state=17):
    ...
    for refactoring in refactorings:
        ...
        for test_size in test_splits:
            ...
            X_train = extract_action_ratios(X_train_seq)
            X_test = extract_action_ratios(X_test_seq)
...
if __name__ == "__main__":
    ...
    with open('cluster_results\\classification_accuracy_ratio_split_single.txt', 'w') as f:
        f.write(str(accs))
    with open('cluster_results\\classification_summary_ratio_split_single.pkl', 'wb') as f:
