...
def run_experiments_sequences(merged_df, refactorings, models, param_grids, test_splits, random_state=17):
    ...
    for refactoring in refactorings:
        ...
        for test_size in test_splits:
            X_train_seq, X_test_seq, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=random_state, stratify=y)
...
if __name__ == "__main__":
    ...
    with open('cluster_results\\classification_accuracy_MEA_single.txt', 'w') as f:
        f.write(str(accs))
    with open('cluster_results\\classification_summary_MEA_single.pkl', 'wb') as f:
