...
def create_action_counts_dataframe(folder_path):
    ...
    df['ratio-node'] = df['insert-node'] / df['delete-node']
    df['ratio-tree'] = df['insert-tree'] / df['delete-tree']
...
def run_experiments(merged_df, refactorings, models, param_grids, test_splits, random_state=17):
    ...
    for refactoring in refactorings:
        ...
        for test_size in test_splits:
            ...
            X_train = merged_df.loc[y_train.index, features]
            X_test = merged_df.loc[y_test.index, features]
...
if __name__ == "__main__":
    ...
    with open('cluster_results\\classification_accuracy_ratiocount_split_single.txt', 'w') as f:
        f.write(str(accs))
    with open('cluster_results\\classification_summary_ratiocount_split_single.pkl', 'wb') as f:
